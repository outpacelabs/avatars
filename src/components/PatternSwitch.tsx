...
/** Round swatch filled with a checkerboard — the dither. */
function DitherIcon() {
	const n = 4;
	const inset = 2.5;
	const cell = 13 / n;
	...
	return (
		<svg
			...
			aria-hidden="true"
		>
			<clipPath id="pf-dither">
				<circle cx="9" cy="9" r="6.5" />
			</clipPath>
			<g clipPath="url(#pf-dither)">{cells}</g>
		</svg>
	);
}
					...
					{OPTIONS.map(({ value: v, label, Icon }) => {
						...
						return (
							<button
								...
								className={`relative flex cursor-pointer items-center gap-2 rounded-full py-2 pl-3 pr-3.5 text-sm font-[550] leading-none transition-colors motion-safe:active:scale-[0.97] ${
									active
										? "text-white/[0.96]"
										: "text-white/[0.48] hover:text-white/[0.8]"
								}`}
							>
								{/* Shared-layout highlight: springs from one option to the
							    other when the selection changes. */}
								{active && (
									<motion.span
